...
# 1. ПОДКЛЮЧЕНИЕ (Убедитесь, что здесь нет лишних символов)
...
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="mn_taxi_nomad")
...
if not st.session_state.auth:
    ...
    if st.session_state.role == "admin":
        ...
        if menu == "🆕 Новый рейс":
            ...
            st.header("Карта активных заказов")
            active = supabase.table("trips").select("*").eq("status", "Новый").execute()
            m = folium.Map(location=[46.3078, 44.2558], zoom_start=6)
