...
# 1. ПОДКЛЮЧЕНИЕ К БАЗЕ (С ЗАЩИТОЙ ОТ СБОЕВ СЕТИ)
URL = "https://hixvwbjybjhyefbsojmm.supabase.co"
KEY = "sb_publishable_dP50ZzMPQaLocvBe6iFxAw_oOvO8Xpm"

try:
    supabase: Client = create_client(URL, KEY, options={"timeout": 20.0})
...
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="mn_nomad_tech_final")
...
if not st.session_state.auth:
    ...
    st.markdown("### Система управления трансферами")
    c1, c2 = st.columns(2)
    ...
    if st.button("ВОЙТИ В СИСТЕМУ", use_container_width=True):
        if l_in == "admin" and p_in == "mn123":
            ...
            except: st.error("Ошибка сети. Попробуйте еще раз.")
    ...
    if st.session_state.role == "admin":
        ...
        if menu == "🆕 Новый рейс":
            ...
            rt = st.text_area("📍 Маршрут (Город, улица)", value=ex['route'] if ex else "")
            pr = st.number_input("💰 Сумма (₽)", value=int(ex['price']) if ex else 4000)
            
            # Выбор водителя из базы
            dr_data = supabase.table("drivers").select("*").execute()
            dr_m = {f"{d['name']} ({d['car']})": d['id'] for d in dr_data.data}
            sl_d = st.selectbox("🚖 Назначить водителя", options=list(dr_m.keys()))
            ...
            for t in active.data:
                ...
                if coords:
                    folium.Marker(coords, popup=f"Рейс: {t['route']}").add_to(m)
            ...
            if res.data:
                if st.button("📊 ВЫГРУЗИТЬ В EXCEL"):
                    output = BytesIO()
                    ...
                    st.download_button("Скачать файл", output.getvalue(), "otchet_mn.xlsx")
                for t in res.data:
            ...
            with st.expander("➕ Регистрация водителя"):
                n, c, l, p = st.text_input("ФИО"), st.text_input("Авто"), st.text_input("Логин"), st.text_input("Пароль")
            ...
            for d in supabase.table("drivers").select("*").execute().data:
                ...
                if c2.button("❌", key=f"dr_rm_{d['id']}"):
    ...
    # --- ИНТЕРФЕЙС ВОДИТЕЛЯ ---
        ...
        dr_o = st.sidebar.radio("Меню", ["🆕 Мои заказы", "📜 Моя история", "Выход"])
