...
@st.cache_resource
def init_connection():
    # Создаем базовый клиент без конфликтных словарей
...
# Безопасный поиск координат
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="mn_transfer_volgograd_final")
...
if not st.session_state.auth:
    ...
    if st.button("ВОЙТИ", use_container_width=True):
        if l_in == "admin" and p_in == "mn123":
            ...
            try:
                ...
                st.error(f"Сервер базы данных недоступен: {e}")
    ...
    if st.session_state.role == "admin":
        ...
        if menu == "🆕 Новый рейс":
            ...
            # Логика редактирования существующего рейса
            ...
            if e_id and 'ed_load' not in st.session_state:
                try:
                    ...
                    st.warning(f"Не удалось загрузить данные рейса: {e}")
            
            if 'p_list' not in st.session_state:
                st.session_state.p_list = [{"name":"","phone":""}]

            # Динамические поля пассажиров
            ...
            rt = st.text_area("📍 Маршрут (Пишите точный адрес для карты)", value=ex['route'] if ex else "")
            pr = st.number_input("💰 Цена", value=int(ex['price']) if ex else 4000)
            
            # Загрузка водителей для выпадающего списка
            sl_d = None
            try:
                ...
                st.warning("Не удалось загрузить список водителей из базы.")
            ...
            if st.button("💾 СОХРАНИТЬ РЕЙС", use_container_width=True):
                if sl_d and rt:
                    ...
                    st.warning("Заполните маршрут и выберите водителя.")
            ...
            try:
                ...
                st.error("Не удалось загрузить активные рейсы.")
            ...
            try:
                ...
                st.error(f"Не удалось загрузить карту: {e}")
            ...
            try:
                ...
                if res.data:
                    if st.button("📊 СКАЧАТЬ В EXCEL"):
                        ...
                        st.download_button("Нажмите для загрузки файла", output.getvalue(), "otchet_mn.xlsx")
                    for t in res.data:
            ...
            try:
                drivers_list = supabase.table("drivers").select("*").execute()
                for d in drivers_list.data:
                ...
                st.error("Не удалось загрузить список водителей.")
        ...
        if dr_o == "🆕 Заказы":
            try:
                ...
                if jobs.data:
                    for j in jobs.data:
                        with st.container(border=True):
                            ...
                            for p in j['passengers']:
                                ...
                                cn.write(f"👤 {p['name']}\n📞 {p['phone']}")
                                cb.markdown(f'<a href="tel:{p["phone"]}"><button style="width:100%;background:#25D366;color:white;border:none;padding:10px;border-radius:5px;cursor:pointer;">📞 Звонок</button></a>', unsafe_allow_html=True)
                    ...
                    st.info("У вас пока нет активных заказов")
            except Exception as e: 
                st.error(f"Ошибка обновления списка заказов: {e}")
